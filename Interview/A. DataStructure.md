...
```
배열로 구현하면 저장 공간이 매우 많이 필요하게 된다.
노드의 깊이에 따라 2의 N승만큼 추가적으로 필요하게 된다.
```
...
```
최소 공통 조상(LCA)이라는 방법이 있다. 노드의 depth를 가지고 공통된 부모 노드와 depth가 얼마나 차이나느냐를 LCA 테이블에 이분 탐색을 이용해 O(NlogN) 안에 구할 수 있게 된다.
```
